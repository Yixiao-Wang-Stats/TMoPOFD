...
```math
\hat{\mu}(t) = \frac{\sum_{i=1}^{n} \mathbf{1}_{[\beta,+\infty)}\left(\text{POIFD_{n}}\left(X_{i}\right)\right)\mathbf{1}_{\text{observed}\left(X_{i}(t)\right)} X_{i}(t)}{\sum_{i=1}^{n} \mathbf{1}_{[\beta,+\infty)}\left(\text{POIFD_{n}}\left(X_{i}\right)\right)\mathbf{1}_{\text{observed}\left(X_{i}(t)\right)}} ,
```

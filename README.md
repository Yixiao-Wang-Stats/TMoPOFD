...
```math
\hat{\mu}(t) = \frac{\sum_{i=1}^{n} \mathbf{1}_{[\beta,+\infty)}\left(\text{POIFD}_{n}\left(X_{i}\right)\right)\mathbf{1}_{\text{observed}\left(X_{i}(t)\right)} X_{i}(t)}{\sum_{i=1}^{n} \mathbf{1}_{[\beta,+\infty)}\left(\text{POIFD}_{n}\left(X_{i}\right)\right)\mathbf{1}_{\text{observed}\left(X_{i}(t)\right)}} ,
```

where $\beta$ satisfies:

```math
\frac{1}{n} \sum_{i=1}^{n} \mathbf{1}_{[\beta,+\infty)}\left(\text{POIFD}\left(X_{i}\right)\right) \approx 1-\alpha ,
```
